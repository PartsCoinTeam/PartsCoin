...
const char * const BITCOIN_CONF_FILENAME = "partscoin.conf";
...
#else
    const char* pszModule = "partscoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PartsCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/PartsCoin";
#else
    // Unix-like
    return pathRet / ".partscoin";
#endif
#endif
}
