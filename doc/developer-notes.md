...
- [Development guidelines](#development-guidelines)
    - [General PartsCoin Core](#general-partscoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
PartsCoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play partscoins" on the test network, if you
...
PartsCoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/PartsCoinTeam/PartsCoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_partscoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_partscoin --log_level=test_suite
$ valgrind -v --leak-check=full src/partscoind -printtoconsole
...
# A coverage report will now be accessible at `./test_partscoin.coverage/index.html`.
```
...
To profile a running partscoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep partscoind` -- sleep 60
```
...
PartsCoin Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as PartsCoin Core, where everyone uses
...
pay attention to for reviewers of PartsCoin Core code.

General PartsCoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: PartsCoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of PartsCoin Core, in which case changes should probably go
directly upstream without being PRed directly against the project. They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project. Changes to these should also
be sent upstream, but bugfixes may also be prudent to PR against PartsCoin Core so that they can be integrated
...
file descriptors, it will cause problems with PartsCoin's `select()` loop, because
...
compatibility between nodes. This happened in PartsCoin 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to PartsCoin's
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `partscoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `partscoin-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of PartsCoin Core, the wallet was always
