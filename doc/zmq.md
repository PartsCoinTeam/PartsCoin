...
The PartsCoin Core daemon can be configured to act as a trusted "border
router", implementing the partscoin wire protocol and relay, making
...
The ZeroMQ feature in PartsCoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building partscoind:
...
For instance:
    ...
    $ partscoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/partscoind.tx.raw \
...
These options can also be provided in partscoin.conf.
...
From the perspective of partscoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into partscoind directly. Furthermore, no information is
...
using. PartsCoind appends an up-counting sequence number to each
